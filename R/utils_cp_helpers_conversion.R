...
#
# helper functions used by 'run_cellpose()' to convert pixel-based ROI measurements
# into physical units based on provided image scale information
#
# No exported functions
#
#
# .cp_convert_roi_units_single()
...
.cp_convert_roi_units_single <- function(stats,
                                         scale_info) {
...
.cp_convert_roi_units <- function(Results_pixel,
  ...
  for(img_name in names(Results_pixel)) {
    ...
    converted <- .cp_convert_roi_units_single(stats, info)
