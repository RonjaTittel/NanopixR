...
setup_env <- function() {
  ...
  # Was this tool already used in this session?
  ...
  # Checking the needed R packages
  ...
  recheck_pkgs <- function() {
    ...
    cat("The following R packages are missing: ", paste(missing, collpase = ", "), "\n", sep = "")
    cat("Please install them using:\n",
        "install.packages(c('", paste(missing, collpase = "', '"), "'))\n", sep = "")
  ...
  # Tool used on this system before
  ...
  # 4) Testing connection to python
  ...
  # 5) Summary of detected Python moduls
